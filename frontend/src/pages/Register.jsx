...
import { Recycle, ArrowRight, User, Mail, Lock, Building } from 'lucide-react';

export default function Register({ setIsLoggedIn, showToast }) {
  const [formData, setFormData] = useState({
    fullName: '',
    email: '',
    password: '',
    role: 'Donor (Individual / Corporate)'
  });
    ...
    <div className="min-h-[calc(100vh-120px)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md glass-card rounded-3xl p-8 border border-slate-800 bg-slate-900/80 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-slate-100">Join ReUseMatch</h2>
          <p className="text-xs text-slate-400 mt-1">Empower circular economy item reuse</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-emerald-500 transition-colors"
              >
                <option value="Donor (Individual / Corporate)">Donor (Individual / Corporate)</option>
                <option value="Recipient (NGO / School / Charity)">Recipient (NGO / School / Charity)</option>
                <option value="Eco-Refurbisher & Recycler">Eco-Refurbisher & Recycler</option>
              </select>
            </div>
          </div>
